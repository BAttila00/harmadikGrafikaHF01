...
//---------------------------
//Autómatikus deriváláshoz fogjuk használni
...
//Ezeket is autómatikus deriváláshoz fogjuk használni
...
//duális szám template struktúra vec2-vel való parametrizálását elnevezzük Dnum2-nek
typedef Dnum<vec2> Dnum2;

//hány sor illetve oszlop lesz a felületekhez tertozó teszellálandó egységnégyzetben lásd 9. diasor 9. dia
...
//---------------------------
	// extrinsic
	vec3 wEye;			//kamera posíció
	vec3 wLookat;		//nézeti irány
	vec3 wVup;			//preferált függöleges irány
	// intrinsic
	float fov;			//függöleges látószög
...
public:
	...
	/// <summary>
	/// Azért transzformáltunk, hogy a láthatósági feladatot és a vetítést képernyö koordinátarendszerben oldhassuk meg
	/// </summary>
	/// <returns></returns>
...
//---------------------------
struct Material {	//inkrementális képszintézisnél diffúz felületekkel dolgozunk többnyire
	//---------------------------

	vec3 kd;	//diffúz visszaverödési tényezö
	vec3 ks;	//spekuláris visszaverödési tényezö
	vec3 ka;	//ambiens visszaverödési tényezö
	float shininess;
};
...
struct Light {
	//---------------------------
	vec3 La;	//ambiens fényforrás (itt most minden fényforráshoz felvett külön ambiens fényt, lehetne úgy is h csak egy ambiens fény van globálisan)
	vec3 Le;	//az adott fényforrás sugársürüsége, intenzitása, fényerössége stb stb
	vec4 wLightPos; //fényforrás helye		// homogeneous coordinates, can be at ideal point	
	//mivel ez egy homogén koordináta, így lehet az ideális pontban (végtelenben) (ha az utsó koordináta nulla)
	//mivel ha egy pontszerü fényforrás a végtelenben van akkor azt irányfényforrásnak vesszük ezért ezzel a struktúrával a pont és irány fényforrások is kezelhetöek
};

//---------------------------
//egy sakktáblatextúrát valósít meg
...
//---------------------------
//egy interfész az objektumaink és a shaderek között
//az objektumok ide teszik be minden olyan adatukat amit áta akarnak adni a shader-nek. A shader meg innen veszi ki ami neki kell
struct RenderState {
	//---------------------------
	mat4 MVP;		//model-view-projection mtx: az objektumot referenciahelyzetböl egyböl normalizált eszközkoordinátarendszerbe viszi (= M * V * P)
	mat4 M;			//modelezési transzform: referenciahelyzetböl világkoordinátarendszerbe visz át
	mat4 Minv;		//M inverze, a modelezési transzformáció során a normálvektorokra alkalmazzuk
	mat4 V;			//view mtx (kamerában definiálva)
	mat4 P;			//projekciós mtx (kamerában definiálva)
	Material* material;
...
class Shader : public GPUProgram {
	//---------------------------
public:
	virtual void Bind(RenderState state) = 0;		//egy Renderstate objektum változóival feltölti az adott shader uniform változóit
...
//---------------------------
class Geometry {		//a geometriai objektumaink alaposztálya (9. diasor 8. dia)
	//---------------------------
protected:
...
public:
	...
	VertexData GenVertexData(float u, float v) {
		...
		vec3 drdU(X.d.x, Y.d.x, Z.d.x);		//u szerinti derivált
		vec3 drdV(X.d.y, Y.d.y, Z.d.y);		//v szerinti derivált
		vtxData.normal = cross(drdU, drdV);	//a normálvektor
		return vtxData;
	}

	//a gpu-ra feltölti a parametrikus felületünk(objektumunk) tesszellált változatát (azaz háromszögek sokaságát)
	//(9. diasor 10 dia)
